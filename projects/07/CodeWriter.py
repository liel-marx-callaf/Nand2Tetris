...
class CodeWriter:
    ...
    # OLD COMPARE FUNCTION - FAILS ON D REGISTER OVERFLOW
    # def _compare(self, command: str) -> str:
    #     true_label = f"TRUE_{command}_{self.label_counter}"
    #     end_label = f"END_{command}_{self.label_counter}"
    #
    #     new_command = (f"// Compare {command}: RAM[SP] {command} RAM[SP-1]\n"
    #                    # D = RAM[SP-1] - RAM[SP]
    #                    "@SP\n"
    #                    "AM=M-1\n"
    #                    "D=M\n"
    ...
    #                    "D=M-D\n"
    #
    #                    # if (RAM[SP-1] - RAM[SP] compares 0) then jump to True label
    #                    f"@{true_label}\n"
    #                    f"D;J{command.upper()}\n"
    #
    #                    # else RAM[SP] = False, then jump to End label
    #                    "@SP\n"
    #                    "A=M-1\n"
    #                    "M=0\n"
    #                    f"@{end_label}\n"
    ...
    # TODO: unused code - I wanted to check stack overflow and underflow, but apparently it is not needed for tests
    # def _stack_underflow_check(self, command: str) -> str:
    #     """
    #     Generates Hack assembly that makes sure we do not decrease the stack when it's empty.
    #
    #     For binary ops (add/sub/and/or/eq/lt/gt), we should have at least two elements in the stack: SP>=258.
    #     For unary ops (neg/not/pop), we should have at least one element in the stack: SP>=257.
    #
    #     If we reach stack underflow, go to infinite loop.
    #
    #     Args:
    #         command (str): which command we want to check.
    #     """
    #     label_id = self.label_counter
    #     self.label_counter += 1
    #     underflow_label = f"STACK_UNDERFLOW_{label_id}"
    #     size = self.STACK_UNDERFLOW_SLOTS[command]  # minimum number of elements required on stack
    #
    #     asm = (
    #         f"// check stack underflow before {command} (need {size} elements)\n"
    #         f"@SP\n"
    #         f"D=M\n"
    #         f"@{size}\n"
    #         f"D=D-A\n"
    #         f"@{underflow_label}\n"
    #         f"D;JLT\n"  # if SP < size, jump to underflow trap
    #         f"({underflow_label})\n"
    #         f"// STACK UNDERFLOW\n"
    #         f"@{underflow_label}\n"
    #         f"0;JMP\n"
    ...
    #     return asm
    #
    # def _stack_overflow_check(self) -> str:
    #     """
    #     Generates Hack assembly that makes sure we do not increase the stack when it's full.
    #
    #     Hack computer has RAM of size 32767.
    #     Only PUSH commands can add to the stack, so check beforehand if out of bounds.
    #
    #     If we reach stack overflow, go to infinite loop.
    #     """
    #     label_id = self.label_counter
    #     self.label_counter += 1
    #     overflow_label = f"STACK_OVERFLOW_{label_id}"
    #
    #     asm = (
    #         f"// check stack overflow before push\n"
    #         f"@SP\n"
    #         f"D=M\n"
    #         f"@32767\n"
    #         f"D=D-A\n"
    #         f"@{overflow_label}\n"
    #         f"D;JGT\n"  # if SP > 32767, jump to overflow trap
    #         f"({overflow_label})\n"
    #         f"// STACK OVERFLOW\n"
    #         f"@{overflow_label}\n"
    #         f"0;JMP\n"
